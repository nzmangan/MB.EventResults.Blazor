...
public class AnalyzerService(ILogger<AnalyzerService> _Logger, AnalyzerServiceConfiguration _AnalyzerServiceConfiguration, IDistanceCalculator _DistanceCalculator) : IAnalyzerService {
    ...
    foreach (var runner in runners) {
      ...
      foreach (var split in runner.Splits) {
        ...
        if (split.PerformanceIndex.HasValue) {
          ...
          if (split.PerformanceIndexAdjusted < _AnalyzerServiceConfiguration.MistakeIndex && totalAverageTimeMapping.TryGetValue(key, out double average)) {
  ...
  public CourseInfo ConvertToControlData(CourseData courseData) {
    ...
    return new() { Controls = controls.Select(c => new ControlData { Lat = c.Position?.Lat, Lng = c.Position?.Lng, Id = c.Id.Value }).ToList() };
  }
